...
main(
    ...
    int nnumas;
    ctu_check(qv_hw_count(base_scope, QV_HW_NUMANODE, &nnumas));
    // Split at NUMA domains.
    qv_scope_t *numa_scope;
    ctu_check(
        qv_split_at(base_scope, QV_HW_NUMANODE, wrank % nnumas, &numa_scope)
    );
    // When there's more tasks than NUMAs,
    // make sure each task has exclusive resources.
    int lrank;
    ctu_check(qv_group_rank(numa_scope, &lrank));

    int ntasks_per_numa;
    ctu_check(qv_group_size(numa_scope, &ntasks_per_numa));
    ...
    int ncores;
    ctu_check(qv_hw_count(subnuma, QV_HW_CORE, &ncores));

    int npus;
    ctu_check(qv_hw_count(subnuma, QV_HW_PU, &npus));
    ////////////////////////////////////////////////////////////////////////////
